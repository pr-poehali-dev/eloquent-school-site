...
function SystemTest() {
  ...
  const processAiRequest = async () => {
    ...
    addLog(`🤖 Claude AI генерирует код...`);

    try {
      const aiResponse = await api.ai.generate(selectedProject.id, aiPrompt);

      if (aiResponse.warning) {
        addLog(`⚠️ ${aiResponse.warning}`);
      }

      if (aiResponse.tokens) {
        const inputCost = (aiResponse.tokens.input / 1000000) * 3;
        const outputCost = (aiResponse.tokens.output / 1000000) * 15;
        const totalCost = inputCost + outputCost;
        addLog(`💰 Токены: ${aiResponse.tokens.input}→${aiResponse.tokens.output} (~$${totalCost.toFixed(4)})`);
      }
      ...
      await api.files.create({
        ...
        type: aiResponse.file_type as ProjectFile['type']
      });
      ...
      addLog(`✨ Создан: ${aiResponse.file_path}`);
      setAiPrompt('');
